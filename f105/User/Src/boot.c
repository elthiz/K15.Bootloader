...
{
	/* Если была попытка записать в бутлоадер, то висим в загрузчике */
	if ( ( 1 << MASK_ERROR_WRITE_IN_BOOT ) & userData.regControl )
	{
		return;
	}
	/* Если была ошибка очистки flash, то висим в загрузчике */
	if ( ( 1 << MASK_ERROR_ERASE ) & userData.regControl )
	{
		return;
	}
	/* Если была ошибка записи в flash, то висим в загрузчике */
	if ( ( 1 << MASK_ERROR_WRITE ) & userData.regControl )
	{
		return;
	}
	/* Если бит "остаться в загрузчике" не выставлен  */
	if ( !( ( 1 << MASK_STAY_BOOT ) & userData.regControl ) )
	{
		/* Если время нахождения в бутлоадере вышло */
...
void jumpToApp( void )
{
	/* Проверка, что приложение валидно */
	if ( !isAppValid() )
	{
		/* Выставляем бит "остаться в загрузчике" */
